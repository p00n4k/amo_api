...
export default function ProjectDetailClient({
    ...
    const uniqueTypes = Array.from(
        new Set(project.collections.map((c) => c.material_type))
    ).sort();

    const filteredCollections = selectedType
        ? project.collections.filter((c) => c.material_type === selectedType)
        : project.collections;

    const nextSlide = () => setCurrent((current + 1) % project.project_images.length);
    const prevSlide = () =>
        setCurrent((current - 1 + project.project_images.length) % project.project_images.length);
    ...
    const getPrevIndex = () =>
        (current - 1 + project.project_images.length) % project.project_images.length;
    ...
    return (
        <div className="bg-[#4a4a4a] min-h-screen text-white overflow-x-hidden">
                    ...
                    <div className="flex justify-center gap-2">
                        {Array.from({ length: totalCarouselPages }).map((_, i) => (
                            <button
                                ...
                                className={`w-3 h-3 rounded-full border-2 border-orange-500 ${i === carouselIndex ? "bg-orange-500" : ""
                                    }`}
                            />
                        ))}
                    </div>
            ...
            <div className="bg-[#3a3a3a] rounded-2xl p-6 mx-8 mb-8 overflow-hidden">
                        ...
                        {dropdownOpen && (
                            <div className="absolute right-0 mt-2 w-48 bg-white text-black rounded shadow-lg z-10">
                                ...
                                {uniqueTypes.map((t) => (
                                    <button
                                        key={t}
                                        className="block w-full px-4 py-2 hover:bg-orange-100 text-left"
                                        onClick={() => {
                                            setSelectedType(t);
                                            setDropdownOpen(false);
                                        }}
                ...
                {/* TODO: render filteredCollections table ของคุณต่อได้เลย */}
            </div>
        </div>
    );
}
